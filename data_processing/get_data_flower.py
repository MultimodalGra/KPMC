...
def split_images():
    # JSON 文件路径
    json_file = "/home/rookie/dataset/oxford_flowers/split_zhou_OxfordFlowers.json"

    # 数据集根目录
    dataset_root = "/home/rookie/dataset/oxford_flowers/jpg"

    # 输出目录
    train_output_dir = os.path.join(dataset_root, "train")
    test_output_dir = os.path.join(dataset_root, "test")
    val_output_dir = os.path.join(dataset_root, "val")
...
#
train_dir = os.path.join('/data2/hh/dataset/oxford_flowers', 'train')
train = datasets.ImageFolder(train_dir, transform=preprocess)
print(train.classes)
test_dir = os.path.join('/data2/hh/dataset/oxford_flowers', 'test')
